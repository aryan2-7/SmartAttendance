...
bool Database::initializeTables() {
    ...
    // Insert a default admin account (only if the table was just created)
    std::string defaultUser =
        "INSERT OR IGNORE INTO users (username, password) "
        "VALUES ('admin', 'admin123');";

    return execute(users) && execute(students) &&
           execute(attendance) && execute(defaultUser);
}
...
bool Database::studentExists(int rollNumber) {
    std::string sql =
        "SELECT COUNT(*) FROM students WHERE rollNumber = " +
        std::to_string(rollNumber) + ";";
